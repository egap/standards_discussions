...
potential_outcomes <- declare_potential_outcomes(Y ~ (1 - Z) * (control_mean + e_0) + Z * (treatment_mean + e_1))
...
cointest <- declare_test(handler=label_test(cointestfn), label="coin")
cluster_two_arm_design <- thepop + potential_outcomes +
    estimand + assignment_m1 + reveal + cointest + stata_est + cr0est + cr2est + olsest
